#!/usr/bin/env python3
"""50.000 başlık embeddingini ayrı Qdrant collection içine yükler."""
...
from .helpers import atomic_json, point_id, read_articles, read_assignments, read_json
from .payloads import build_title_payload as build_payload
from .validation import validate_collection, validate_vector_inputs

ARTICLES_PATH = (
    ROOT / "data" / "processed" / "final_articles_50000.jsonl"
)
...
PROGRESS_PATH = (
    ...
    / "final_50k"
    / "search"
    / "qdrant_title_index_progress.json"
)
...
MANIFEST_PATH = (
    ...
    / "final_50k"
    / "search"
    / "qdrant_title_index_manifest.json"
)

validate_inputs = partial(
    validate_vector_inputs, quality_path=QUALITY_SUMMARY_PATH,
    metadata_path=EMBEDDING_METADATA_PATH, vector_label="Başlık embedding",
...
def main() -> None:
    ...
    expected_count = int(
        config.get("target_article_count", 50000)
    )
    ...
    vector_size = int(
        qdrant_config.get("vector_size", 768)
    )
    ...
    batch_size = int(
        args.batch_size
        or qdrant_config.get("batch_size", 128)
    )

    timeout_seconds = int(
        qdrant_config.get("timeout_seconds", 180)
    )

    print("=" * 80)
    print("QDRANT 50.000 BAŞLIK İNDEKSLEME")
    print("=" * 80)
    ...
    try:
        ...
        if args.recreate and exists:
            print("\nMevcut title collection siliniyor...")
        ...
        if not exists:
            print("\nTitle collection oluşturuluyor...")
            ...
            if (
                ...
                if (
                    ...
                    print(
                        "Geçerli 50.000 title point mevcut; "
                        "yeniden yüklenmedi."
                    )
                    ...
                    raise RuntimeError(
                        "Collection ve dataset hashleri "
                        "eşleşmiyor. --recreate kullanın."
                    )
                ...
                if current_count != start_row:
                    raise RuntimeError(
                        "Qdrant count ve progress uyuşmuyor.\n"
                        "Qdrant : %d\n"
                        "Progress: %d"
                        % (current_count, start_row)
                    )
                ...
                raise RuntimeError(
                    "Collection kısmen dolu fakat progress "
                    "dosyası yok. --recreate kullanın."
                )
        ...
        for batch_start in range(
            ...
            print(
                "Title Qdrant ilerleme: %d/%d | geçen %.1f sn"
                % (
                    batch_end,
                    expected_count,
                    time.time() - started,
        ...
        atomic_json(
            MANIFEST_PATH,
            {
                ...
                "text_field": "title_tr",
                "embedding_path": str(
                    EMBEDDINGS_PATH.relative_to(ROOT)
                ),
                "articles_path": str(
                    ARTICLES_PATH.relative_to(ROOT)
                ),
                "assignments_path": str(
                    ASSIGNMENTS_PATH.relative_to(ROOT)
                ),
                "payload_indexes": [
                    {
                        "field_name": field_name,
        ...
        print("\n" + "=" * 80)
        print("TITLE QDRANT İNDEKSLEME TAMAMLANDI")
        print("=" * 80)
        print("\nExact point sayısı:", final_count)
